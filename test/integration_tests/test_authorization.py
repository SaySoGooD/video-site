...
class TestAdminAccess:
    ...
    def test_default_role_is_403(self, client: TestClient) -> None:
        ...
        assert client.get(f"{API}/admin/users", headers=auth_header(token)).status_code == 403
    ...
    def test_moderator_lacking_the_permission_is_403(
        ...
        assert client.get(f"{API}/admin/roles", headers=auth_header(token)).status_code == 403

    def test_admin_is_allowed(self, client: TestClient) -> None:
        token = login(client, "admin@example.com", "admin123")
        response = client.get(f"{API}/admin/roles", headers=auth_header(token))
    ...
    def test_granting_a_role_changes_what_a_user_may_do(
        ...
        roles = client.get(f"{API}/admin/roles", headers=admin).json()
        ...
        assert client.get(f"{API}/admin/users", headers=user).status_code == 200
