...
class LogoutUseCase(ILogoutUseCase):
    ...
    Logout is server-side: stamping ``revoked_at`` makes the still-unexpired
    JWT unusable on the next request. An already-invalid token simply has
    nothing to revoke, so logout is idempotent and never errors.
    """
    ...
    async def __call__(
        ...
        async with self._uow as uow:
            await uow.sessions.revoke(payload.jti, datetime.now(UTC))
            await audit.record(
                uow,
                AuditAction.LOGOUT,
